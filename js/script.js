  ...
  for (let i = 0; i < moviesContainer.length; i++) {
    ...
    <div class="item text-center">
      <img src="https://image.tmdb.org/t/p/w500/${
        moviesContainer[i].poster_path
      }" class="w-100" alt="${moviesContainer[i].original_title}">
      <div class="cap p-2">
        <h3>${moviesContainer[i].original_title}</h3>
        <p>${moviesContainer[i].overview.slice(0, 200)}...</p>
...
$("#btn").on("click", function () {
  if ($("#sideBar").css("left") === "0px") {
    ...
    for (let i = 0; i < 5; i++) {
      $("#sideBar ul li").eq(i).animate({ opacity: 0 ,paddingTop: 70},(i+5) * 180)
    }
  } else {
    ...
    for (let i = 0; i < 5; i++) {
      $("#sideBar ul li").eq(i).animate({ opacity: 1 ,paddingTop: 10},(i+5) * 180)
    }
  }
});
